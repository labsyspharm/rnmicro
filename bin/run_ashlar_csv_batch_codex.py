...
def replace_destain(fpath):
    return str(fpath).replace('destain', 'zdestain')
...
for exp in exps[args.from_dir : args.to_dir]:
    ...
    #[TODO]: xml for CODEX data
    files_exp = sorted(raw_dir.rglob('*_info.xml'))
    file_type = '_info.xml'
    if len(files_exp) == 0:
        files_exp = sorted(raw_dir.glob('*xdce'))
        file_type = 'xdce'
    files_exp.sort(key=replace_destain)

    if len(files_exp) == 0:
        print('No {} files found in'.format(file_type), str(raw_dir))
        continue
    ...
    if text_to_bool(exp['Correction']):
        ...
        for j in files_exp:
            ...
            if (path_exp / 'illumination_profiles' / ffp_file_name).exists() and (path_exp / 'illumination_profiles' / dfp_file_name).exists():
                ...
                tiffs = j.parent.glob('*.tif')
                pattern_j = re.sub('c\d+', 'c{channel}', str(tiffs.next()))
                pattern_j = re.sub('m\d+', 'm{tile}', pattern_j)
                #[TODO]: below call is different:  download script
                call("C:\Users\Public\Downloads\Fiji.app\ImageJ-win64.exe --ij2 --headless --run C:\Users\Public\Downloads\Fiji.app\plugins\imagej_basic_ashlar_filepattern.py \"pattern='%s', output_dir='%s', experiment_name='%s', lambda_flat=%s, lambda_dark=%s\"" %(pattern_j, str(illumination_dir), j.name.replace(file_type, ''), lambda_flat, lambda_dark))
    ...
    #[TODO]:separate input_files (different from typical)
    input_files = ' '.join(['"zen|{}"'.format(str(f)) for f in files_exp])

    #[TODO]: parameters to add (unique for Denis' CODEX)
    # --filter-sigma = 1
    # -m 150 (is for 20x magnification)

    command = 'ashlar ' + input_files + ' -m 150 --filter-sigma 1 -o ' + str(out_dir)

    if text_to_bool(exp['Pyramid']):
        command += ' --pyramid -f ' + path_exp.name + '_m150_sigma1_5b1a06.ome.tif'
    
    if text_to_bool(exp['Correction']):
        ffps = ' '.join(['"{}"'.format(ffp) for ffp in ffp_list])
        dfps = ' '.join(['"{}"'.format(dfp) for dfp in dfp_list])
